...
import com.rentaherramientas.tolly.application.dto.auth.*;
import com.rentaherramientas.tolly.application.usecase.auth.*;
...
public class AuthController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Usuario creado exitosamente", content = @Content(schema = @Schema(implementation = UserFullResponse.class))),
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Usuario obtenido exitosamente", content = @Content(schema = @Schema(implementation = UserFullResponse.class))),
                        @ApiResponse(responseCode = "401", description = "No autenticado"),
                        @ApiResponse(responseCode = "404", description = "Usuario no encontrado")
        })
        @SecurityRequirement(name = "bearerAuth")
        @GetMapping("/me")
        public ResponseEntity<UserFullResponse> getCurrentUser(Authentication authentication) {
                UUID userId = (UUID) authentication.getPrincipal();
                UserFullResponse response = getCurrentUserUseCase.execute(userId);
                return ResponseEntity.ok(response);
        }
